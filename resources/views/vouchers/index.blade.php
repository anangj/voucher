                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select name="status" id="status" class="form-control">
                                            <option value="assign" {{ request('status') == 'active' ? 'selected' : '' }}>Assign</option>
                            ...
                            <table class="w-full divide-y table-fixed divide-slate-100 dark:divide-slate-700 data-table">
                                <thead class="bg-slate-200 dark:bg-slate-700">
                                    <tr>
                                        ...
                                        <th scope="col" class="table-th"> {{__('QrCode')}} </th>
                                        <th scope="col" class="table-th"> {{__('Patient')}} </th>
                                        <th scope="col" class="table-th"> {{__('Expiry Date')}} </th>
                                        <th scope="col" class="table-th"> {{__('Status')}} </th>
                                        {{-- <th scope="col" class="table-th"> {{__('Max Uses')}} </th> --}}
                                        {{-- <th scope="col" class="table-th"> {{__('Current Uses')}} </th> --}}
                                ...
                                <tbody class="bg-white divide-y divide-slate-100 dark:divide-slate-700 dark:bg-slate-800">
                                    ...
                                    <tr>
                                        <td class="table-td">{{ $voucher->voucher_no }}</td>

                                        <td class="table-td">{{ $voucher->voucherHeader->paketVoucher->name ?? 'N/A' }}</td>

                                        {{-- Display QR Code --}}
                                        <td class="table-td">
                                            @if ($voucher->qr_code)
                                                <img src="data:image/svg+xml;base64,{{ $voucher->qr_code }}" alt="QR Code" class="w-16 h-16">

                                            @else
                                                {{ __('No QR Code') }}
                                            @endif
                                        </td>
                                        <td class="table-td">{{ $voucher->voucherHeader->patient->name }}</td>
                                        <td class="table-td">{{ $voucher->voucherHeader->expiry_date }}</td>
                                        <td>
                                            @if ($voucher->voucherHeader->status == 'expired')
                                                <div class="inline-block px-3 min-w-[90px] text-center mx-auto py-1 rounded-[999px] bg-opacity-25 text-warning-500
                                                    bg-warning-500">
                                                    {{ ucfirst($voucher->voucherHeader->status) }}
                                                </div>
                                            @elseif ($voucher->voucherHeader->status == 'active')
                                        ...
                                        {{-- <td class="table-td">{{ ucfirst($voucher->voucherHeader->status) }}</td> --}}
                                        {{-- <td class="table-td">{{ $voucher->max_uses }}</td> --}}
                                        {{-- <td class="table-td">{{ $voucher->current_uses }}</td> --}}
